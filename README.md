# 👋 Hey there, I'm Saad Ansari!

🚀 **Senior Frontend & Full Stack Engineer** | 🧠 **Exploring GenAI** | 💻 **Code • Deploy • Repeat**

---

### 👨‍💻 About Me

I'm a passionate engineer with 8+ years of experience in building scalable web applications. Currently diving deep into the world of **Generative AI**, while sharpening my skills in **Python**, **SQL** and **AWS**.

- 💼 Working on backend APIs.
- ⚡ Love optimizing performance
- 🧠 Actively learning and experimenting with GenAI to build smart, interactive tools

---

### 🛠 Tech Stack

**Languages & Frameworks:**  
 `JavaScript` `React` `Nodejs` `SQL`

**Web & Cloud**  
`HTML` `CSS` `AWS (EC2, S3, IAM, Lambda)`  
`GitLab CI/CD` `SonarQube`

**AI/ML & Learning:**  
`GenAI` `LangChain` `LLMs` `Python (basics)` `n8n`

---

### 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ansarisaad11&show_icons=true&theme=tokyonight" alt="Stats" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=ansarisaad11&theme=tokyonight" alt="Streak" />
</p>

---

### 🔍 Currently Working On

- 🤖 A **GenAI-powered chatbot** for answering R&D questions from research papers and patents

---

### 📫 Let's Connect!

- [LinkedIn - @saadansari](https://www.linkedin.com/in/saadansari115/)
- 📬 Reach me at: `saadansari1105@gmail.com`

---
